...
def test_version_agreement_rejects_package_from_wrong_origin(tmp_path, monkeypatch):
    ...
    with pytest.raises(ReleaseArtifactError, match="resolved outside") as captured:
        _validate_version_agreement(project, "1.0.1.dev0")
...
def test_frozen_v050_evidence_is_verified_independently_of_active_version(tmp_path):
    ...
    assert project_version(ROOT) == "1.0.1.dev0"
...
def test_frozen_v060_evidence_is_verified_independently_of_active_version(tmp_path):
...
def test_frozen_v060_evidence_rejects_publication_identity_changes(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    evidence_path = tmp_path / "docs" / "v0.6.0_release_evidence.json"
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    evidence["tag"]["target"] = "0" * 40
    evidence_path.write_text(json.dumps(evidence), encoding="utf-8")

    with pytest.raises(ReleaseArtifactError, match="publication evidence changed"):
        verify_frozen_v060_evidence(tmp_path)


def test_frozen_v070_evidence_is_verified_independently_of_active_version(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    ...
    assert result["release"]["pypi_published"] is False
    assert result["tag"]["target"] == result["release_pr"]["squash_commit"]
    assert result["manual_validation"]["stable_artifact_smoke_test"] == "PASS"
    assert set(result["assets"]) == {
...
def test_frozen_v070_evidence_rejects_publication_identity_changes(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    evidence_path = tmp_path / "docs" / "v0.7.0_release_evidence.json"
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    evidence["tag"]["target"] = "0" * 40
    evidence_path.write_text(json.dumps(evidence), encoding="utf-8")

    with pytest.raises(ReleaseArtifactError, match="publication evidence changed"):
        verify_frozen_v070_evidence(tmp_path)


def test_frozen_v080_evidence_is_verified_independently_of_active_version(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
...
def test_frozen_v080_evidence_rejects_publication_identity_changes(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    evidence_path = tmp_path / "docs" / "v0.8.0_release_evidence.json"
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    evidence["tag"]["target"] = "0" * 40
    evidence_path.write_text(json.dumps(evidence), encoding="utf-8")

    with pytest.raises(ReleaseArtifactError, match="publication evidence changed"):
        verify_frozen_v080_evidence(tmp_path)


def test_frozen_v090_evidence_is_verified_independently_of_active_version(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    ...
    assert result["tag"]["target"] == result["release_pr"]["squash_commit"]
    assert result["manual_validation"]["stable_artifact_smoke_test"] == "PASS"
    assert result["manual_validation"]["owner_verdict"] == "READY TO PUBLISH v0.9.0"
    assert set(result["assets"]) == {
...
def test_frozen_v090_evidence_rejects_publication_identity_changes(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    evidence_path = tmp_path / "docs" / "v0.9.0_release_evidence.json"
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    evidence["tag"]["target"] = "0" * 40
    evidence_path.write_text(json.dumps(evidence), encoding="utf-8")
...
def test_frozen_v100_evidence_is_verified_independently_of_active_version(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    result = verify_frozen_v100_evidence(tmp_path)
    assert project_version(ROOT) == "1.0.1.dev0"
    assert result["version"] == "1.0.0"
    assert result["release"]["draft"] is False
    assert result["release"]["prerelease"] is False
    assert result["release"]["pypi_published"] is False
    assert result["tag"]["annotated"] is True
    assert result["tag"]["target"] == result["release_pr"]["squash_commit"]


def test_frozen_v100_evidence_rejects_byte_changes(tmp_path):
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    evidence = tmp_path / "docs" / "v1.0.0_release_evidence.json"
    evidence.write_bytes(evidence.read_bytes() + b"\n")

    with pytest.raises(ReleaseArtifactError, match="Frozen v1.0.0 publication evidence changed"):
        verify_frozen_v100_evidence(tmp_path)
